  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm rounded-3xl">
        ...
        {/* Inner form is keyed on `open` so it remounts (and resets its
            internal `amount`/`showSuccess` state) every time the dialog
            re-opens. This avoids the `setState`-in-`useEffect` pattern that
            React 19 / Next 16 forbids. */}
        {open && (
          <TransferForm
            key={`${fromBank.id}->${toBank.id}`}
            fromBank={fromBank}
            toBank={toBank}
            maxAmount={maxAmount}
            currency={currency}
...
function TransferForm({ fromBank, toBank, maxAmount, currency, onConfirm }: TransferFormProps) {
  const [amount, setAmount] = useState('');
  const [showSuccess, setShowSuccess] = useState(false);
  const inputRef = useRef<HTMLInputElement>(null);

  // Focus the amount input when the form mounts (= each time the dialog opens).
  // No setState here, so this doesn't trip the `set-state-in-effect` lint rule.
  useEffect(() => {
    // Small delay so the Dialog's mount animation finishes first.
